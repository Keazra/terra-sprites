# Terra Sprites — M1 "A Sprite Lives" design (v2)

- **Status:** Draft v2 for review
- **Date:** 2026-09-23
- **Supersedes:** [v1](m1-a-sprite-lives.md)
...
- **Blocked re-planning:**
  - After 3 consecutive blocked ticks, the sprite re-plans with **currently occupied tiles treated as impassable**.
  - If a path to a goal tile still exists, the sprite follows it. If not (for example, a sprite resting in the only corridor), **the action ends with outcome `blocked`**, freeing the brain to choose again.
  - Retreat and Wander follow the same rule. For Retreat, the equivalent of "no path" is "no free neighbour increases the distance".
...
| Pleased | `♥` | `+` | `Tickled`, or `last_r` above the UI's spike threshold |
...
1. **Unit tests (`terra-sim`):**
   ...
   - **World:**
     ...
     - blocked re-planning with occupied tiles impassable, ending in `blocked`
...
| A2 | 1 sprite; balls, food, water | Each applied Play on a Ball at tick *t* < 9,900 → `Tickle(actor)` stamped *t+1* | Applied Play-on-Ball count ≥ 1.5× control |
| A3 | 4 sprites in a small arena; food, water | Each applied Hit on a Sprite at tick *t* < 9,900 → `Slap(actor)`, meaning **the hitter**, stamped *t+1* | Applied Hit-on-Sprite count, all sprites combined, ≤ 0.5× control |

...
- **The current design doc** in `docs/design/`: for each design, the highest-numbered `-vN` revision is current (today, `m1-a-sprite-lives-v4.md`). Read the sections that touch the area you're about to work in. Earlier revisions and the `*-eval.md` files are history, not spec.
